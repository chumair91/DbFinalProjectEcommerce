...
const AddCopy = ({ token }) => {
  ...
  useEffect(() => {
    ...
    // For each image, create a preview URL ONLY if image exists
    if (image1) urls.push(URL.createObjectURL(image1));
    if (image2) urls.push(URL.createObjectURL(image2));
    if (image3) urls.push(URL.createObjectURL(image3));
    if (image4) urls.push(URL.createObjectURL(image4));
  ...
  return (
    <form onSubmit={onSubmitHandler} className='flex flex-col w-full items-start gap-3'>
      ...
      <div className='w-full'>
        ...
        <input onChange={(e) => { setName(e.target.value) }} value={name} className='w-full max-w-[500px] px-3 py-2 ' type="text" placeholder='Enter Name' required />
      </div>
      ...
      <button className='bg-black text-white px-8 py-2 group active:bg-black/70' type='submit'><span className=' inline-block group-active:scale-95 transform transition'>ADD</span></button>
